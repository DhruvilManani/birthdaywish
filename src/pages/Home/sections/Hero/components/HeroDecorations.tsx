  ...
  return (
    <div className="absolute inset-0 pointer-events-none z-20 overflow-hidden">
      ...
      <motion.div 
        ...
        className="absolute top-[20%] left-[15%] opacity-60 flex gap-1 transform-gpu"
        animate={{ y: [0, -15, 0], x: [0, 10, 0], rotate: [-5, 5, -5] }}
        transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }}
      >
        <div className="w-3 h-4 rounded-full bg-rose-light/50 " />
        <div className="w-2 h-3 rounded-full bg-rose-light/40  mt-1" />
      </motion.div>

      <motion.div 
        style={{ x: invParallaxX, y: invParallaxY }} 
        className="absolute top-[35%] right-[18%] opacity-50 flex gap-1 transform-gpu"
        animate={{ y: [0, -12, 0], x: [0, -10, 0], rotate: [0, -10, 0] }}
        transition={{ duration: 5, repeat: Infinity, ease: "easeInOut", delay: 1 }}
      >
        <div className="w-2 h-3 rounded-full bg-golden/40 " />
        <div className="w-3 h-4 rounded-full bg-golden/50  mt-1" />
      </motion.div>

      {/* Abstract Floating Petals (Replaces emoji roses) */}
      <motion.div 
        className="absolute top-[60%] left-[25%] opacity-40 transform-gpu"
        animate={{ y: [0, -30, 0], rotate: [-20, 20, -20], scale: [1, 1.2, 1] }}
        transition={{ duration: 7, repeat: Infinity, ease: "easeInOut" }}
      >
        <div className="w-4 h-4 rounded-br-full rounded-tl-full bg-gradient-to-br from-rose-light to-transparent " />
      </motion.div>
      
      <motion.div 
        className="absolute top-[25%] right-[25%] opacity-30 transform-gpu"
        animate={{ y: [0, -20, 0], rotate: [0, 45, 0] }}
        transition={{ duration: 8, repeat: Infinity, ease: "easeInOut", delay: 2 }}
      >
         <div className="w-5 h-5 rounded-bl-full rounded-tr-full bg-gradient-to-bl from-soft-pink-light to-transparent " />
      </motion.div>

      {/* Elegant Floating Orbs */}
      <motion.div 
        className="absolute top-[75%] right-[15%] w-16 h-16 rounded-full bg-gradient-to-tr from-rose-light/10 to-golden/10 border border-white/20 shadow-[0_0_20px_rgba(255,255,255,0.1)] backdrop- transform-gpu"
      ...
      >
        <div className="w-40 h-40 -mt-20 -ml-20 bg-golden/15 rounded-full  transition-opacity duration-500" />
      </motion.div>
